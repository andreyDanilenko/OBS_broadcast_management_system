...
This project automates cartoon broadcasting through OBS Studio. It scans the local media library, builds a single daily schedule, and manages the `VideoSource` input inside OBS to play the correct episode at the right time. The toolkit includes three console utilities:
...
- `parse_schedule` — interactive CLI from `generate/` that builds `schedule_day.json`.

All tools operate on a single `schedule_day.json` file that stores the playback order from 06:00 through 24:00.
...
- `generate/main.go` — source code of `parse_schedule`, creates `schedule_day.json`.
- `cartoons/` — media library of cartoons.
- `cartoons.json`, `schedule_day.json`, `state.json` — shared data artifacts.
...
- `schedule_day.json` (produced by `parse_schedule`)
...
4. The script fills a single day schedule (06:00–24:00). Keep entering cartoons and counts until the available time is filled; the program notifies you when the day is full or no more slots remain.
5. The resulting `schedule_day.json` is saved alongside the binary.
...
2. Ensure the working directory contains:
   ...
   - `cartoons.json`, `schedule_day.json`, `state.json` (the last file appears after the first run);
   - the `cartoons/` folder with media files.
3. Start `cartoon-scheduler`. The console prints `http://localhost:8080`.
4. Open the web interface: “Start schedule” kicks off playback according to `schedule_day.json`; “Scan cartoons” invokes `cartoon-scanner`; “Stop” terminates the loop.
...
3. **Create the daily schedule**  
   Run `./parse_schedule` and keep entering `cartoon_id count` pairs until the entire day is filled; the tool outputs `schedule_day.json`.
...
- **Sample `schedule_day.json` produced by `parse_schedule`**
